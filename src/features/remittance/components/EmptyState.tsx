...
export function EmptyState({ onPressCta }: Props) {
  ...
  return (
    ...
    >
      <Ionicons name="paper-plane-outline" size={36} color={colors.textSecondary} />
      <Text style={[typography.body, { color: colors.text, textAlign: 'center', fontWeight: '600' }]}>
        {t('remittance.empty.title')}
      </Text>
      <Text style={[typography.caption, { color: colors.textSecondary, textAlign: 'center' }]}>
        {t('remittance.empty.body')}
      </Text>
      <Pressable
        accessibilityRole="button"
        accessibilityLabel={t('remittance.empty.cta')}
        onPress={onPressCta}
        style={({ pressed }) => ({
