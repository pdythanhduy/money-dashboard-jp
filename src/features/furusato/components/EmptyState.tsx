...
export function EmptyState({ onPressCta }: Props) {
  ...
  return (
    ...
    >
      <Ionicons name="gift-outline" size={36} color={colors.textSecondary} />
      <Text
        style={[typography.body, { color: colors.text, textAlign: 'center', fontWeight: '600' }]}
      >
        {t('furusato.empty.title')}
      </Text>
      <Text
        style={[typography.caption, { color: colors.textSecondary, textAlign: 'center' }]}
      >
        {t('furusato.empty.body')}
      </Text>
      <Pressable
